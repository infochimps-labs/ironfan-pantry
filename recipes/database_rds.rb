...
aws_rds_instance "#{node[:cluster_name]}-zabbix" do
  username node.zabbix.database.root_user
  password node.zabbix.database.root_password
  action   :create
end

# Discover the RDS instance to use
zabbix_database_host = zabbix_rds_instance_hostname

# Establish root MySQL connection
root_mysql_conn = {:host => zabbix_database_host, :username => node.zabbix.database.root_user, :password => node.zabbix.database.root_password}
...
begin
  ...
  Gem.clear_paths  
  require 'mysql'
  m=Mysql.new(zabbix_database_host,node.zabbix.database.root_user,node.zabbix.database.root_password)
  if m.list_dbs.include?(node.zabbix.database.name) == false
    ...
    execute "zabbix_populate_schema" do
      command "/usr/bin/mysql -h #{zabbix_database_host} -u #{node.zabbix.database.root_user} #{node.zabbix.database.name} -p#{node.zabbix.database.root_password} < /opt/zabbix-#{node.zabbix.server.version}/create/schema/mysql.sql"
      action :nothing
    end
    execute "zabbix_populate_data" do
      command "/usr/bin/mysql -h #{zabbix_database_host} -u #{node.zabbix.database.root_user} #{node.zabbix.database.name} -p#{node.zabbix.database.root_password} < /opt/zabbix-#{node.zabbix.server.version}/create/data/data.sql"
      action :nothing
    end
    execute "zabbix_populate_image" do
      command "/usr/bin/mysql -h #{zabbix_database_host} -u #{node.zabbix.database.root_user} #{node.zabbix.database.name} -p#{node.zabbix.database.root_password} < /opt/zabbix-#{node.zabbix.server.version}/create/data/images_mysql.sql"
      action :nothing
    end
